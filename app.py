...
def _strava_auth_url():
    client_id, _, redirect_uri, scope = _strava_config()
    if not redirect_uri:
        return None
...
def _strava_refresh_token_if_needed():
    ...
    if now_ts < float(exp) - 30:
        return
    client_id, client_secret, _, _ = _strava_config()
    data = {
        "client_id": client_id,
        "client_secret": client_secret,
        "grant_type": "refresh_token",
    ...
    r.raise_for_status()
    new_tok = r.json()
    st.session_state["strava_token"] = new_tok

def _strava_api_get(path, params=None):
    _strava_refresh_token_if_needed()
    tok = st.session_state.get("strava_token")
    if not tok:
        return None
    headers = {"Authorization": f"Bearer {tok.get('access_token')}"}
    r = requests.get("https://www.strava.com/api/v3" + path, headers=headers, params=params)
    r.raise_for_status()
    return r.json()
...
def render_strava_tab(user_id: str):
    ...
    if not athlete:
        try:
            athlete = _strava_api_get("/athlete")
        except Exception:
            athlete = None
    if athlete:
        st.subheader(f"{athlete.get('firstname','')} {athlete.get('lastname','')}")
        st.json({"athlete_id": athlete.get("id")})
    col1, col2 = st.columns(2)
    if col1.button("Sair do Strava"):
        _strava_logout()
