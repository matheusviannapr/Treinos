    ...
    if page == "Dashboard":
        ...
        with col2:
            if st.button("🧪 Gerar rascunho"):
                ...
                save_all(df_new)
                st.success("Rascunho gerado e salvo.")
        with col3:
        ...
        base_load = st.number_input("Carga base (proxy)", 0.0, 500.0, 50.0, step=1.0)
        st.markdown("#### Proporções da carga")
        base_split = {}
        for mod in MODALIDADES:
            val = base_split.get(mod, 0.0)
            default = {"Corrida": 0.35, "Ciclismo": 0.40, "Natação": 0.15, "Força/Calistenia": 0.07, "Mobilidade": 0.03}.get(mod, 0.0)
            base_split[mod] = st.number_input(f"{mod} (%)", 0.0, 1.0, default, 0.01)
        if st.button("📈 Gerar ciclo"):
            ...
            def week_targets(load, split):
                ...
                for mod, frac in split.items():
                    part = load * frac
                    if mod == "Natação": out[mod] = round(part * 1000)
                    elif mod in ("Força/Calistenia", "Mobilidade"): out[mod] = round(part * 60)
                    else: out[mod] = round(part, 1)
                return out
