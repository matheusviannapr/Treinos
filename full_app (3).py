...
def main():
    ...
    if page == "Dashboard":
        ...
        if st.button("📈 Gerar ciclo"):
            # Regenerate all weeks in the cycle
            ...
            for i in range(n_weeks):
                ...
                if i < max(1, n_weeks - 3):
                    if i < (n_weeks // 2):
                        load = base_load * (1 + 0.05 * i)  # Base phase: gradual build
                    else:
                        load = base_load * (1 + 0.05 * (n_weeks // 2) + 0.07 * (i - (n_weeks // 2)))  # Build phase
                elif i == n_weeks - 2:
                    load = base_load * 1.20  # Peak
                else:
                    load = base_load * 0.65  # Recovery
            ...
            st.success(
                "Ciclo gerado e salvo com sucesso. Acesse as semanas geradas na aba 'Planejamento semanal'."
            )
